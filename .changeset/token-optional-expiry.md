...
`POST /api/v1/tokens` now answers `400` for an `expires_in_days` that is negative, not a number, or too large to be a date. A negative count used to create a token that was already expired, and a value too large for a date failed with an error instead of a response. Omitting it or sending `0` still means no expiry, and there is still no upper limit through the API.
