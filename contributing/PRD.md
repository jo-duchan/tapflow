...
tapflow는 iOS/Android QA이 시뮬레이터·에뮬레이터를 브라우저에서 직접 실행할 수 있게 해주는 오픈소스 라이브러리다. 별도의 클라우드 인프라 없이 개발자의 Mac을 서버로 활용하며, 릴레이 서버도 팀이 직접 셀프호스팅한다.

### 1.2 핵심 가치

| 가치 | 설명 |
|------|------|
| Zero infra cost | 유저가 이미 보유한 Mac을 서버로 활용 |
| Zero data leak | 앱과 데이터가 외부 클라우드로 나가지 않음 |
| Zero config | `npx tapflow deploy` 한 줄로 릴레이 서버 배포 |
| Multi-platform | iOS(Mac), Android(Mac) 동일 인터페이스로 지원 |
| Open source | 완전 오픈소스, 커스터마이징 가능 |

### 1.3 타겟 유저

- iOS/Android 앱을 개발하는 스타트업·중소 팀
- QA이 별도로 있으나 디바이스·인프라 비용을 줄이고 싶은 팀
...
| Xcode 시뮬레이터 직접 | QA이 Mac/Xcode 직접 설치 필요 |
| ios-bridge (오픈소스) | LAN 수준 접속, 유지보수 불확실 (Star 42) |
| baguette (오픈소스) | iOS 26 전용, 팀 협업 레이어 없음 |

tapflow가 해결하는 것: 개발자 Mac의 시뮬레이터/에뮬레이터를 QA이 브라우저에서 안전하게 접근할 수 있게 하고, 팀 워크플로우(버그 리포트, 세션 공유, 테스트 케이스)까지 제공한다.
...
**멀티 Mac 풀**: 여러 Mac이 동일한 relay에 연결하면 대시보드에서 모든 Mac의 시뮬레이터가 하나의 디바이스 목록으로 표시된다. QA원 각각이 서로 다른 Mac의 시뮬레이터를 동시에 독립적으로 사용할 수 있다. 추가 설정 없이 agent를 실행하고 같은 relay URL을 지정하면 자동으로 풀에 합류한다.

### 3.2 컴포넌트 역할

| 컴포넌트 | 역할 | 기술 스택 |
|---------|------|----------|
| DeviceAgent (iOS) | 시뮬레이터 제어, 화면 캡처, 터치 인젝션 | Node.js + xcrun simctl + SimDeviceLegacyHIDClient + IndigoHID (Swift) |
| DeviceAgent (Android) | 에뮬레이터 제어, ADB 래핑 | Node.js + ADB |
| 릴레이 서버 | NAT 통과, 세션 라우팅, JWT 인증, 데이터 저장 | Node.js + ws + SQLite |
| Web Dashboard | QA UI, 세션 뷰어, 테스트 관리 | Vite + React 19 + TypeScript |
...
| Android | macOS 필수 | ❌ Mac 인스턴스 필요 | ADB + Android Emulator | Mac Mini 중고 $300~ |
...
**터치 인젝션 (SimDeviceLegacyHIDClient + IndigoHID)**

`touch-helper` Swift 바이너리가 `CoreSimulator.framework`을 dlopen으로 로드해 `SimDeviceLegacyHIDClient`를 획득하고, stdin 프레임을 읽어 즉시 HID 이벤트로 변환·주입한다. WebDriverAgent는 사용하지 않는다.
...
stdin 프로토콜 (가변 길이 프레임):
  ...
  type 1 = touch start  (x, y 정규화 0–1)
  type 2 = touch move
  type 3 = touch end
  type 4 = HID button   (a=usagePage, b=usage)
  type 5 = legacy button (a=code)  ← 홈 버튼
  type 6 = pinch start  (x1,y1 = finger0, x2,y2 = finger1)
  type 7 = pinch move
  type 8 = pinch end
  type 9 = key press    ([0]=modifierBitmap, [4–7]=hidUsage u32BE)
```

키보드 주입은 `IndigoHIDMessageForKeyboardArbitrary(usage, op)` 경로를 사용한다. `IndigoHIDMessageForHIDArbitrary`(digitizer 경로)는 iOS가 하드웨어 키보드로 인식하지 못해 한/영 전환이 동작하지 않으므로 사용하지 않는다.

**Android 터치 인젝션 ✅ Phase 3 완료**

scrcpy control 소켓의 바이너리 프로토콜(`ScrcpyControl.touchDown/touchMove/touchUp`)을 우선 사용한다. scrcpy 세션이 없는 경우에만 `adb shell input`으로 폴백한다.
...
Agent가 릴레이로 먼저 outbound WebSocket 연결을 맺고 대기한다. 방화벽/NAT 문제가 없다. QA 브라우저는 릴레이에만 접속하면 된다.
...
| `agent:resources` | Agent → Relay | CPU·RAM 사용률 보고 (5초 주기, cpuPercent, memUsedMB, memTotalMB, slotsAvailable, slotsTotal) |
...
| `agent_resources` | 4 | Mac 자원 시계열 (agent_name, cpu_percent, mem_percent, recorded_at) — 1분 집계, 30일 보존 |
...
| 커맨드 | 설명 |
|--------|------|
| `tapflow start` | 로컬 개발 전용 — relay + agent 동시 기동 (같은 Mac) |
| `tapflow relay start [--port]` | relay만 기동 (Docker/Linux 서버용) |
| `tapflow agent start [--relay] [--platform] [--device]` | agent만 기동 — 기존 relay에 연결 |
| `tapflow init [--relay]` | relay 최초 관리자 계정 생성 |
| `tapflow doctor` | 시스템 prerequisites 점검 (Xcode, simctl, adb 등) |
| `tapflow devices` | 사용 가능한 시뮬레이터·AVD 목록 |
| `tapflow boot <name>` | 이름 또는 UDID로 시뮬레이터 부팅 |
| `tapflow reset` | 모든 시뮬레이터·에뮬레이터 종료 |
| `tapflow status [--relay]` | 연결된 agent·디바이스·세션 수 표시 |
| `tapflow logs [--relay] [--lines]` | relay 인메모리 로그 조회 |

```bash
# 로컬 개발 (relay + agent 동시 기동)
tapflow start
...
> **미구현 (로드맵)**: `tapflow deploy` (인프라 배포), `tapflow upload` (빌드 업로드 CLI) — Phase 5 예정. 빌드 업로드는 현재 REST API(`POST /api/v1/builds`)로만 가능하다.

### 6.2 API 엔드포인트 (PAT 인증)
...
여러 대의 Mac을 RAID처럼 묶어 단일 디바이스 풀로 운영한다. QA은 어느 Mac의 시뮬레이터인지 신경 쓰지 않고 사용하고, 시스템이 자원 상황에 맞게 배분한다.
...
- **자원 사용량 시계열 그래프 ✅ Phase 4-A 완료** (PR #21): 릴레이가 1분 집계 데이터를 DB에 저장(30일 보존), `/mac-resources` 대시보드 페이지에서 Mac별 CPU·RAM 추이 Area 차트 조회 (1h / 6h / 24h / 7d 범위 선택) — 어드민의 Mac 추가·회수 판단 근거 제공
...
| Mac 전원 유지 | QA 사용 중 iOS Agent Mac이 켜져 있어야 함 |
