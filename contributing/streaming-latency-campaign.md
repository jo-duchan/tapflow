...
- **2026-06-03 — freeze 단축 (A 비트레이트 캡 + B IDR-on-drop):** PR-D 스모크에서 찢어짐은 사라졌으나 드롭 후 **짧은 freeze(렉 체감)** 잔존. 진단: freeze = ①**혼잡 지속**(iOS 인코더에 비트레이트 캡 부재 → 스크롤 ~20Mbps 버스트가 LAN 초과 → 지속 백프레셔) + ②**다음 IDR까지 대기**(주기 2s). **A:** VideoToolbox `AverageBitRate`+`DataRateLimits` 캡(`TAPFLOW_IOS_H264_BITRATE`, 기본 8Mbps=Android와 동일) → 혼잡 자체 축소. **B:** relay의 keyframe-aware sender가 버퍼 회복했는데 키프레임 없으면 `onWantKeyframe`→relay가 agent에 `stream:request-idr`(throttle 500ms)→ios-agent→swift stdin `0x01`→다음 프레임 강제 IDR(기존 ForceKeyFrame 재사용) → 혼잡 풀리면 ~즉시 재동기(2s 대기 제거). Android는 scrcpy 프로토콜이라 IDR 요청 미지원(메시지 무시, 무해). 단위테스트 +2(onWantKeyframe), 전 패키지 tsc/lint·테스트 통과(agent-core 23·relay 168·ios 153). swift 재컴파일. **디바이스 검증(2026-06-03): freeze 사라짐. 단 `DataRateLimits`(하드 캡)가 고모션에서 프레임 손상(찢어짐) 유발 → 제거하고 `AverageBitRate`(소프트 타겟)만 유지.** 재검증: 찢어짐 없음·freeze 짧음. 교훈: **하드 비트레이트 캡(DataRateLimits) 금지, AverageBitRate만.**
