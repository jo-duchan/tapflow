...
- **The Android agent's log says why the scrcpy server exited** ([#481](https://github.com/jo-duchan/tapflow/issues/481)). When the server process died on the device, the video socket closed and the stream restarted, which is right — but the log showed the restart and nothing about the cause, so a session that kept restarting because the device had been unplugged or the server was being killed on the device could not be told apart from one restarting for no reason. The exit code, or the signal, is now recorded next to it. A server the agent stopped itself is logged at debug level only, so routine teardown does not print an error-shaped line.
