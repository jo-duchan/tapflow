...
import type { DeviceInfo, RelayMessage, SessionInfo } from '@/lib/types'

type Build = { id: number; name: string; version_label: string | null; status_label: string | null; platform: string }
...
export function QASession() {
  ...
  const handleMessage = useCallback((msg: RelayMessage) => {
    if (msg.type === 'agents:listed') {
      ...
      if (!deviceId && msg.sessions.length > 0) {
        ...
        if (first) { setOs(first.platform); setDeviceId(first.id) }
      }
    }
  ...
  const osVersions = [...new Set(filteredDevices.map((d) => d.osVersion).filter(Boolean))] as string[]
  ...
  function handleLaunch() {
    if (!activeSessionId || !build) return
    send({ type: 'app:launch', sessionId: activeSessionId, buildId: build.id })
  }
          ...
          <div className="flex w-full items-center gap-3">
            ...
            {build && (
              <span className="text-sm text-muted-foreground">
                {build.name}{build.version_label ? ` · ${build.version_label}` : ''}
              </span>
            )}
            <div className="flex-1" />
            <Button size="sm" variant="outline" onClick={handleLaunch}>
              <Play className="mr-1 h-4 w-4" />Launch app
            </Button>
          </div>
          <SimulatorViewer sessionId={activeSessionId} onBack={() => setActiveSessionId(null)} />
        </div>
        <Separator orientation="vertical" className="h-auto" />
        <div className="w-80 shrink-0">
          {buildId && <CommentPanel buildId={Number(buildId)} />}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg">
      ...
      <div className="flex flex-col gap-4">
        <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Select device</h2>

        <div className="flex flex-col gap-3">
          <div className="grid gap-1.5">
            <span className="text-sm font-medium">OS</span>
            <Select value={os} onValueChange={(v) => { setOs(v); setDeviceId(''); setOsVersion('') }}>
          ...
          {osVersions.length > 0 && (
            <div className="grid gap-1.5">
              ...
              <Select value={osVersion} onValueChange={(v) => { setOsVersion(v); setDeviceId('') }}>
                <SelectTrigger><SelectValue placeholder="Any version" /></SelectTrigger>
                <SelectContent>
          ...
          <div className="grid gap-1.5">
            ...
            {versionedDevices.length === 0 ? (
              ...
              <Select value={deviceId} onValueChange={setDeviceId}>
                ...
                <SelectContent>
                  {versionedDevices.map((d: DeviceInfo) => (
                    <SelectItem key={d.id} value={d.id}>
                      {d.name}
                      {d.status === 'booted' && <span className="ml-1 text-xs text-muted-foreground">(booted)</span>}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
        ...
        <Button onClick={handleBoot} disabled={!deviceId || booting || !connected} className="w-full">
          {booting ? 'Booting…' : selectedDevice?.status === 'booted' ? 'Connect' : 'Boot & Connect'}
        </Button>
      </div>
    </div>
  )
