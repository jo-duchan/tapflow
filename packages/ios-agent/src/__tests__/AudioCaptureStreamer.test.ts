...
import { parseAudioFrames, AudioCaptureStreamer, applyGain, readSimVolume } from '../AudioCaptureStreamer'
...
describe('AudioCaptureStreamer (loopback TCP → AudioFrame stream)', () => {
  it('listens on an ephemeral port and yields AudioFrames for PCM a client writes', async () => {
    ...
    // a fake helper: connect to the loopback port and write two length-prefixed PCM frames
