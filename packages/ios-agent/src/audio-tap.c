...
// AudioQueue path (AudioServicesPlaySystemSound, AVAudioPlayer on some paths, raw AudioQueue apps).
...
__attribute__((constructor)) static void audio_tap_init(void) {
    ...
    g_sock = s;
    fprintf(stderr, "audio-tap: connected to 127.0.0.1:%d (interposing AudioQueueEnqueueBuffer)\n", port);
}
