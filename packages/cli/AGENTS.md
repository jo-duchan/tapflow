...
- `agent start --token` (or `TAPFLOW_AGENT_TOKEN`) carries an `agent`-scope PAT, required when the relay is on a different machine; flag wins over env. Its "create an agent-scope PAT" hint follows every 1008 except the owner refusal (`no longer an Admin`), whose reason already names the fix — a new token from a current Admin; the relay's reason is printed as-is either way.
