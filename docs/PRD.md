...
| App Center | `/app-center` | App 목록 사이드바(bundle_id 기반) · Release Accordion(version_name 그룹) · Build 카드 + Start QA CTA, 수동/API 업로드, 상태 태그, 검색·필터·페이지네이션 |
| QA Session | `/app-center/build?id=` | OS·OS 버전·Device 선택 → 부팅 → `.app.zip` 자동 설치, 시뮬레이터 뷰어 + 터치, 코멘트(이미지 첨부·deep-link 공유) |
...
Phase 2 완료 후 도메인 모델 재설계. 덩치가 커지기 전에 마이그레이션하는 것이 비용이 적다.

- **iOS 빌드 포맷**: `.ipa` → `.app.zip` (시뮬레이터 표준 포맷, Appetize·Firebase App Distribution 동일)
  - 업로드 시 `lipo -info`로 시뮬레이터 슬라이스(x86_64 또는 arm64-simulator) 검증
  - 디바이스 슬라이스만 있으면 400 + 재빌드 안내 반환
- **DB 재설계**: `apps`(앱 엔티티) + `builds`(빌드 산출물) 분리
  - `apps`: `id, name, bundle_id_key, platform, created_at`
  - `builds`: `id, app_id FK, version_name, build_number, bundle_id, status_label, file_path, uploader_id, uploaded_at`
  - `version_name`(CFBundleShortVersionString) · `build_number`(CFBundleVersion) 업로드 시 자동 추출
  - `bundle_id_key`로 App 자동 생성·조회 (업로드 시 신규 bundle_id면 `apps` row 생성)
- **App Center UI 재설계**:
  - 좌측: App 목록 사이드바 (bundle_id 기반)
  - 중앙: Release Accordion (version_name으로 그룹, 같은 version_name의 build들을 묶어 표시)
  - Build 카드: version_name · build_number · status · Start QA CTA
- **API 추가**: `GET /api/v1/apps` — App 목록 + latest_build 요약
...
- **WebRTC DataChannel 터치**: UDP 기반으로 전환해 WAN 환경 터치 레이턴시 개선
...
- **디바이스 풀 대시보드**: Mac별 가용 슬롯 수, 현재 점유자(누가 어떤 기기를 쓰는지), 대기열
- **세션 시작 옵션 — 디바이스 초기화 모드**: QA 세션 시작 시 초기화 수준을 선택할 수 있도록 제공
  - `앱 데이터만 초기화` (기본값, Phase 3~에서 적용 중): 해당 앱만 `simctl uninstall` 후 재설치
