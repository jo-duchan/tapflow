...
확장은 tapflow와 함께 옵니다. 설치와 승인 절차는 [iOS 네트워크 확장](/ko/operate/network-extension#network-not-set-up)에 있고, 모두 에이전트 맥에서 합니다. 승인 단계에는 관리자 암호가 필요합니다.
...
오프라인으로 확인하던 중에 기기가 스스로 온라인으로 돌아왔다는 알림이 뜬다면, 그때까지 확인한 오프라인 동작은 다시 확인해야 합니다. 원인과 대처는 [문제 해결](/ko/operate/network-extension#network-stopped)에 있습니다.
