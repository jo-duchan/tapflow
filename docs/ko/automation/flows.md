...
플로우는 사람이 처음부터 손으로 작성하는 언어가 아니라, 코딩 에이전트가 생성하거나 시연에서 뽑아내는 산출물입니다. 에이전트로 작성하는 방법은 [CI/CD에서 MCP 활용](/ko/automation/mcp-ci)을 참고하세요.
:::
...
작성은 에이전트가, 재생은 결정적 러너가 맡는 구조는 [CI/CD에서 MCP 활용](/ko/automation/mcp-ci)에서 더 자세히 다룹니다.
