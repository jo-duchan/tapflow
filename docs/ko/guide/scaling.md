...
다른 Mac에서 `tapflow agent start`를 실행할 때 `ws://localhost:4000`은 릴레이가 있는 Mac이 아니라 해당 Mac 자신의 localhost를 가리킵니다. 릴레이의 실제 네트워크 주소를 사용하세요. 같은 내부 네트워크라면 로컬 IP(`ws://192.168.x.x:4000`), 외부에서 접근한다면 공개 URL이 필요합니다. [릴레이 배포](/ko/guide/self-hosting)를 참고하세요.
:::
