...
tapflow의 모든 API 접근은 **Personal Access Token(PAT)** 으로 제어됩니다.

- PAT는 팀원별로 개별 발급됩니다. 팀원이 퇴사하면 해당 토큰만 폐기합니다.
- 각 토큰에는 **scope**가 지정됩니다. `read-only` 토큰은 스트림을 볼 수 있지만 빌드를 올리거나 기기를 제어할 수 없습니다.
- CI/CD 파이프라인용 토큰은 `upload` scope만 부여해 최소 권한으로 유지할 수 있습니다.

| Scope | 허용 작업 |
|-------|-----------|
| `admin` | 팀 관리, 토큰 발급, 전체 설정 |
| `upload` | 빌드 파일 업로드 |
| `stream` | 기기 세션 접근, 터치 입력 |
| `read` | 빌드 목록 조회, 스크린샷 |
