...
릴레이는 자기가 실행 중인 Mac에만 이 로그를 보여 주므로 이 명령은 릴레이 Mac에서 실행합니다. 다른 Mac에서 원격 릴레이를 가리키면 릴레이가 `403`으로 거절하고 CLI는 릴레이 Mac에서 할 일을 안내합니다. Docker로 운영하면 컨테이너 밖의 CLI도 원격으로 취급되니 `docker compose logs`를 쓰세요.
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `--relay <url>` | `http://localhost:<local.port>` (기본 4000) | 같은 Mac의 릴레이 URL. 릴레이를 다른 포트로 띄웠을 때 지정합니다. `relay.url` 설정은 읽지 않습니다. |
