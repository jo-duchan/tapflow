...
이 페이지는 tapflow 스트리밍을 **실제로 측정한 기록**입니다. 모든 수치는 측정 조건과 함께 제시하며, 끝의 [직접 재현하기](#직접-재현하기)로 같은 측정을 직접 돌려볼 수 있습니다. 마케팅 수치가 아니라, 어떤 조건에서 무엇이 나왔는지를 그대로 적습니다.
...
`glass-to-glass`는 캡처 시각(에이전트 머신)과 표시 시각(뷰어 머신)의 뺄셈이라, 두 머신의 시계가 다른 LAN 환경에서는 직접 측정할 수 없습니다. 그래서 LAN에서는 어느 환경에서나 유효한 `decode→present`를 측정하고, 전체 체감은 [추정](#체감-지연-추정)으로 다룹니다.
...
- 해상도 다운스케일은 대역폭과 디코드 부하를 줄이는 대신 화질을 일부 희생합니다(선택적, 기본은 네이티브).
...
브라우저에서 `?perf=1`과 `?decoder=`(`wasm` / `webcodecs` / `mse` / `jpeg`)를 붙여 티어별로 비교합니다. 크로스 머신 LAN 측정은 같은 LAN의 다른 Mac에서 뷰어를 띄워 측정합니다.
