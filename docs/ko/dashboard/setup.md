...
배포 방식별 자세한 내용은 [릴레이 배포](/ko/guide/self-hosting)를 참고하세요. 팀원이 어느 경로로 접속하느냐에 따라 받는 스트림 품질도 달라집니다. 같은 LAN이라도 평문 HTTP면 **Standard**, HTTPS면 더 선명한 **Sharp** 프로파일이 적용됩니다. 접속 환경별 프로파일은 [스트림 품질](/ko/guide/streaming)을 참고하세요.
