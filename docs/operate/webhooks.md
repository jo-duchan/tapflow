...
To add one at runtime, use `POST /api/v1/webhooks`. Authentication is the same as build upload — a personal access token (PAT) with the `builds:write` scope. See [Build Distribution](/operate/ci-distribution) for token generation.
...
This is an outbound notification tapflow sends. The opposite direction — getting a build *into* tapflow in the first place — is a normal build upload, covered in [Build Distribution](/operate/ci-distribution).
